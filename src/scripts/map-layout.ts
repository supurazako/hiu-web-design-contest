...
export const createMapLayoutController = ({
  ...
  const syncBottomOverlayLayout = () => {
    ...
    refs.mapAttribution.classList.toggle("is-raised-by-selection", hasSelectedSpot);
    ...
    refs.root.style.setProperty("--map-attribution-measured-height", `${measuredHeight}px`);
    refs.root.style.setProperty("--map-selected-sheet-top", `${selectedSheetTop}px`);
  };
  ...
  const updateCompareUI = () => {
    ...
    refs.splitHandle.setAttribute("aria-valuenow", String(Math.round(state.splitRatio * 100)));
    refs.splitHandle.setAttribute("aria-valuetext", `${Math.round(state.splitRatio * 100)}%`);
    refs.splitHandle.setAttribute("aria-orientation", isVerticalCompare ? "vertical" : "horizontal");
    ...
    refs.clockDial.setAttribute("aria-valuenow", String(Math.round(state.clockHour * 10) / 10));
    refs.clockDial.setAttribute("aria-valuetext", clockTimeText);
    if (state.displayMode === "magnifier") {
      ensureMagnifierPoint();
      refs.root.style.setProperty("--magnifier-x", `${state.magnifierPoint.x}px`);
      refs.root.style.setProperty("--magnifier-y", `${state.magnifierPoint.y}px`);
    }
    ...
    refs.controlShell.classList.toggle("is-compare", state.displayMode === "compare");
    refs.controlShell.classList.toggle("is-magnifier", state.displayMode === "magnifier");
    refs.controlShell.classList.toggle("is-clock", state.displayMode === "clock");
